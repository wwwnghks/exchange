...
<script type="text/javascript">
	...
	function listnum(page) {
		location.href = "/exchange/qna/list?curPage=" + page + "&searchOption=${map.searchOption}" + "&keyword=${map.keyword}";
	}
</script>
<style>
.layer {
...
<body>
	...
	<table border="1" width="600px" class="layer">
		<tr>
			<th>번호</th>
			<th>제목</th>
			<th>이름</th>
		...
		<c:forEach varStatus="status" var="row" items="${map.list}">
			<tr>
				...
				<td><input type="hidden" name="mem_idx" value="${session_member.mem_idx}">
				<a href="view?qa_idx=${row.qa_idx}&curPage=${map.qnaPager.curPage}&searchOption=${map.searchOption}&keyword=${map.keyword}">${row.qa_title}</a>
				</td>
		...
		<tr>
			<td colspan="5">
				...
				<c:forEach var="num" begin="${map.qnaPager.blockBegin}" end="${map.qnaPager.blockEnd}">
					<!-- **현재페이지이면 하이퍼링크 제거 -->
					<c:choose>
