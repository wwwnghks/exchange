...
CREATE TABLE `favorite_product` (
  `fav_pro_idx` int(11) NOT NULL AUTO_INCREMENT COMMENT '관심상품번호',
  `pro_idx` int(11) NOT NULL COMMENT '물품번호',
  `mem_idx` int(11) NOT NULL COMMENT '회원번호',
  PRIMARY KEY (`fav_pro_idx`),
  KEY `FK_product_TO_favorite_product` (`pro_idx`),
  KEY `FK_member_TO_favorite_product` (`mem_idx`),
  CONSTRAINT `FK_member_TO_favorite_product` FOREIGN KEY (`mem_idx`) REFERENCES `member` (`mem_idx`),
  CONSTRAINT `FK_product_TO_favorite_product` FOREIGN KEY (`pro_idx`) REFERENCES `product` (`pro_idx`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='관심상품';
